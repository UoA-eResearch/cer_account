...
public class EmailUtil {
	 ...
	 */
	public void sendAccountRequestEmail(AccountRequest ar, Integer dbAccountId,
		...
		try {
			this.templateEmail.sendFromResource(this.emailFrom, this.emailTo,
					null, null, this.accountRequestEmailSubject,
					this.accountRequestEmailBodyResource, templateParams);
		} catch (Exception e) {
			log.error("Failed to send account request email", e);
			throw new Exception(
					"Failed to notify CeR staff about the new account request");
		}
	}

	public void sendAccountDetailsChangeRequestRequestEmail(Person op, Person np)
			throws Exception {
		...
		templateParams.put("__OLD_INSTITUTIONAL_ROLE__", dbDao
				.getInstitutionalRoleName(op.isResearcher() ? op
						.getInstitutionalRoleId() : -1));
		...
		templateParams.put("__NEW_INSTITUTIONAL_ROLE__",
				dbDao.getInstitutionalRoleName(np.getInstitutionalRoleId()));
		...
		try {
			this.templateEmail.sendFromResource(this.emailFrom, this.emailTo,
					null, null, this.accountChangeRequestEmailSubject,
					this.accountChangeRequestEmailBodyResource, templateParams);
		} catch (Exception e) {
			log.error("Failed to send account details change request email", e);
			throw new Exception(
					"Failed to notify CeR staff about the account details change request");
		}
	}

	public void sendAccountDeletionRequestEmail(Person p) throws Exception {
		...
		try {
			this.templateEmail.sendFromResource(this.emailFrom, this.emailTo,
					null, null, this.accountDeletionRequestEmailSubject,
					this.accountDeletionRequestEmailBodyResource,
					templateParams);
		} catch (Exception e) {
			log.error("Failed to send account deletion request email.", e);
			throw new Exception(
					"Failed to notify CeR staff about the account deletion request.");
		}
	}
	...
	public void setAdviserBaseUrl(String adviserBaseUrl) {
	...
	public String getAccountRequestEmailSubject() {
		return accountRequestEmailSubject;
	}

	public String getAccountChangeRequestEmailSubject() {
		return accountChangeRequestEmailSubject;
	}

	public String getAccountDeletionRequestEmailSubject() {
		return accountDeletionRequestEmailSubject;
	}
	...
	public void setAccountRequestEmailSubject(String accountRequestEmailSubject) {
